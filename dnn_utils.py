...
def relu(Z):
    ...
    cache -- returns Z; stored for computing the backward pass efficiently
    """
...
def softmax(Z):
    ...
    Arguments:
    Z - Ouput of the linear layer of any shape
    
    Returns:
    A-- Post-activation parameter, or the same shape as Z
    cache -- returns Z; stored for computing the backward pass efficiently
    """
    
    expo = np.exp(Z)
    
    A = expo/(np.sum(expo, axis = 0, keepdims = True))
...
def softmax_backward(dA, cache):
    ...
    s = expo/(np.sum(expo, axis = 0, keepdims = True))
